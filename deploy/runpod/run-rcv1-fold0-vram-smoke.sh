#!/usr/bin/env bash
# Run inside the HGBL tmux pane. Stage canonical RCV1 if needed, then run a
# short fold-0 execution solely to observe actual GPU/VRAM behaviour.
set -euo pipefail
...
printf 'run_id=%s\ndataset=%s\nfold=%s\nbatch_size=%s\ntraining_steps=%s\nlabel_cpt_steps=%s\nrepo_commit=%s\n' \
    "$RUN_ID" "$DATASET" "$FOLD" "$BATCH_SIZE" "$TRAINING_STEPS" "$LABEL_CPT_STEPS" \
